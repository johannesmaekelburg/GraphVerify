...
def ProProcessingStep1(file_name):
    file_namefull = folder_path + str(file_name)
    preTree = ET.parse(file_name)
...
def yarrrmlparser_bash():
    p = Popen([folder_path + 'yarrrmlparser.sh', folder_path], stdout = PIPE , stderr = PIPE)
    p.communicate()
    p.wait()

def rmlmapper_bash():
    p = Popen([folder_path + 'rmlmapper.sh', folder_path], stdout = PIPE , stderr = PIPE)
    p.communicate()
    p.wait()
...
def validates(process):
    ...
    for j in range(len(message_lines)):
        ...
        message = [part.strip() for part in parts]
        message_new = message[0].replace('Message:', '')
        number = j 
        dynamic_item = [number, message_new]
        tabelle.append(dynamic_item)
